@php
    $heading = __('shop.orders.paid.subject');
    $introLines = [
        __('shop.orders.paid.intro', ['number' => $order->number]),
        __('shop.orders.paid.next'),
    ];
    $buttonUrl = config('app.url');
    $buttonLabel = __('shop.orders.paid.button');
...
>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
        ...
        @endif
        <tr>
            <td style="padding:10px 0;border-bottom:1px solid #f1f1f1;color:#666;font-size:12px;text-transform:uppercase;letter-spacing:0.03em;">{{ __('shop.common.amount_due') }}</td>
        ...
        <tr>
            ...
            <td align="right" style="padding:10px 0;border-bottom:1px solid #f1f1f1;font-size:14px;color:#0b7a29;font-weight:700;">{{ __('shop.common.paid') }}</td>
        </tr>
        @if($order->paid_at)
            <tr>
                <td style="padding:10px 0;color:#666;font-size:12px;text-transform:uppercase;letter-spacing:0.03em;">{{ __('shop.common.paid_at') }}</td>
