@php
    ...
    $heading = __('shop.auth.welcome.subject', ['app' => $appName]);
    $introLines = [
        __('shop.auth.greeting', ['name' => $user->name]),
        __('shop.auth.welcome.intro', ['app' => $appName]),
    ];
@endphp

<x-emails.auth.layout
    :title="__('shop.auth.welcome.title', ['app' => $appName])"
    :heading="$heading"
    :intro-lines="$introLines"
    :button-url="$verificationUrl"
    :button-label="__('shop.auth.welcome.button')"
>
    @if ($verificationUrl)
