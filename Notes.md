Мета та контекст
Сформувати гнучкий роудмап ручного тестування адмін-панелі (Filament-based Mine panel) з урахуванням того, що нею активно користуватимуться реальні менеджери магазину.

Етап 0. Підготовка
Збір інформації та артефактів

Актуальна документація, вимоги до ролей, бізнес-процеси.

Список підключених інтеграцій (Stripe, Meilisearch, MinIO тощо) та критичність для адмінки.

Налаштування середовищ

Переконатися, що dev-середовище відповідає prod (включно з міграціями, тестовими даними, правами доступу).

Домовленість про тестову обліковку(и) з різними ролями.

Етап 1. Базова регресія інтерфейсу
Smoke-тести навігації

Доступність панелі, логін/логут, перемикання мов, базовий рендер усіх головних розділів.

Перевірка критичних CRUD-потоків

Створення/редагування/видалення ключових сутностей (товари, категорії, замовлення, користувачі).

Переконатися, що валідація форм відображає зрозумілі повідомлення користувачу.

Перевірка масових дій та фільтрів

Пошук, сортування, фільтри, пагінація, масові операції (активація/деактивація, експорт тощо).

Контроль віджетів/дашбордів

Оновлення статистики, коректність чисел, реакція на різні часові періоди.

Етап 2. Розширена функціональна перевірка
Ролі та права доступу

Перевірити обмеження функціоналу для кожної ролі (менеджер контенту, оператор замовлень, адміністратор).

Спроба доступу до заборонених розділів через прямі URL.

Інтеграції та зовнішні сервіси

Stripe: створення/оновлення замовлення, відстеження статусів, робота з помилками вебхуків.

Meilisearch/повнотекстовий пошук: релевантність результатів, поведінка при пустих/невалідних запитах.

MinIO/S3: завантаження/відображення медіа, обмеження по розміру/формату.

Черги, повідомлення, нотифікації

Відстеження черги завдань, статусів відправки листів, повторні спроби.

Перевірка відображення системних повідомлень користувачу.

Етап 3. Нефункціональні аспекти
Юзабіліті

Зрозумілість текстів, локалізація, логічність розміщення елементів, доступність клавіатурою.

Перевірка поведінки на популярних розширеннях екрана, responsive-дизайн (якщо панель використовується на планшетах).

Продуктивність

Час завантаження списків при великих об’ємах даних, наявність прогрес-індикаторів.

Поведінка при повільній мережі / обриві інтернету.

Безпека

Повторний логін після зміни пароля, таймаути сесії, CSRF/валідація форм.

Видимість сенситивних даних (маскування номерів карток, логів).

Етап 4. Підтримка регресії та зміни
Каталог тест-кейсів

Структура: розділ адмінки → основні кейси → негативні сценарії.

Для кожного кейсу: попередні умови, кроки, очікуваний результат, пріоритет, статус автоматизації.

Регресійні чеклісти

Швидкі чеклісти на кожен реліз (Smoke + високопріоритетні кейси).

Окремі чеклісти для критичних інтеграцій (платежі, пошук, зберігання файлів).

Додаткові нотації

Теги для позначення зон ризику (наприклад, #payments, #inventory, #internationalization).

Ведення журналу знайдених дефектів з прив’язкою до чеклістів.

Етап 5. Постійне вдосконалення
Зворотній зв’язок від користувачів

Опитування менеджерів, аналіз запитів у support/Slack.

Додавання нових кейсів за результатами інцидентів.

Синхронізація з автоматизацією

Позначати, які ручні кейси покриваються автотестами, та переносити рутину в automation backlog.

Аналіз метрик

Час виконання регресії, кількість багів у релізі, повторюваність помилок.

План дій для зменшення ручного навантаження без втрати покриття.

Формат підтримки роудмапу
Документ/таблиця зі структурою «Етап → Задачі → Статус → Коментарі».

Kanban-дошка для планування хвиль тестування (підготовка даних, виконання, ретест).

Виробнича дисципліна: оновлювати після кожного релізу, зберігати історію змін, позначати відповідальних.

Подальше розширення
Додати специфічні сценарії для нових модулів адмінки.

Вести перелік відкритих питань/ризиків, які треба уточнити з командою.

Інтегрувати з calendar/roadmap розробки, щоби заздалегідь планувати регресію.
