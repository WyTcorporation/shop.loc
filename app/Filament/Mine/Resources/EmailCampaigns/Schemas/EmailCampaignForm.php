...
namespace App\Filament\Mine\Resources\EmailCampaigns\Schemas;
    ...
    public static function configure(Schema $schema): Schema
    {
        ...
        return $schema->components([
            Section::make(__('Campaign details'))
                ->schema([
                    Hidden::make('type')
                        ->default(MarketingCampaign::TYPE_EMAIL)
                        ->dehydrateStateUsing(fn () => MarketingCampaign::TYPE_EMAIL),
                    TextInput::make('name')
                        ->label(__('Name'))
                        ->required(),
                    Select::make('template_id')
                        ...
                        ->options(fn () => CampaignTemplate::query()
                            ->where('channel', MarketingCampaign::TYPE_EMAIL)
                            ->orderBy('name')
                            ->pluck('name', 'id')
                            ->toArray())
                        ->searchable()
